            ...
            <View style={styles.meInfoWrap}>
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.edit}>
                  <Text style={{ fontWeight: "500",  color:"#1de9b6", marginTop: 10 }}>{item.name}</Text>
                </View>
                <View
                  style={{
                    height: 1,
                ...
                <View style={styles.meData}>
                  <View style={styles.data}>
                    <Text style={{ fontSize: 12, fontWeight: "600" , color:"#f5f5f5"}}>{item.eventscount}</Text>
                    <Text style={{ fontSize: 12, color: "#e0e0e0" }}>events</Text>
                  </View>

                  <View style={styles.data}>
                    <Text style={{ fontWeight: "600",  color:"#f5f5f5" }}>{item.followers}</Text>
                  ...
                  <TouchableOpacity onPress={()=>{this.pressedLike()}} >
                    ...
                    <MaterialCommunityIcons
                      color={this.state.touchableHighlightMouseDown?'#f5f5f5':(this.state.liked ? '#dd2c00' : '#f5f5f5')}
                      name="heart-outline"
                      size={20}
                    />
                    <Text style={{ fontSize: 12, color: "#e0e0e0" }}>{this.state.touchableHighlightMouseDown?'follow':(this.state.liked ? 'unfollow' : 'follow')}</Text>
